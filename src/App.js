...
const [START_AMOUNT, DEPOSIT_AMOUNT, WITHDRAW_AMOUNT, LOAN_AMOUNT] = [
	500, 150, 50, 5000,
];
	...
	switch (action.type) {
		...
		case "payLoan":
			return {
				...state,
				balance:
					state.balance > state.loan ? state.balance - state.loan : 0,
				loan:
					state.balance > state.loan ? 0 : state.loan - state.balance,
			};
		case "close":
			return {
				...state,
